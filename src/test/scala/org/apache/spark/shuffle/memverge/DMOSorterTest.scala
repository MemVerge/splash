...
class DMOSorterTest {
  ...
  private var sorter: DMOSorter[Int, Int, Int] = _
  private var strSorter: DMOSorter[String, String, String] = _
  ...
  private def newSparkContextWithForceSpillSize(
    ...
    conf.set(MVFSOpts.forceSpillElements, size)
  ...
  def testCleanupIntermediateFilesInSorter(option: FailuresOption): Unit = {
    ...
    val sorter = new DMOSorter[Int, Int, Int](
    ...
    assertThat(sorter.numSpills) isGreaterThan 0
    assertThat(DMOTmpFile.listAll()) isNotEmpty()
    sorter.stop()
  ...
  def testCleanupIntermediateFilesInShuffle(option: FailuresOption): Unit = {
    ...
    TestUtil.assertSpilled(sc) {
      if (withFailures) {
        ...
        assertThat(new DMOFile(sc.applicationId).list()) hasSize 2
      } else {
        assertThat(data.reduceByKey(_ + _).count()) isEqualTo size
        // contains shuffle data and index for two slices
        assertThat(new DMOFile(sc.applicationId).list()) hasSize 4
      }
    }
  }
  ...
  def testSorter(option: SorterTestOption): Unit = {
    ...
    val size = 1000
    if (withSpilling) {
      conf.set(MVFSOpts.forceSpillElements, size / 2)
    } else {
      conf.set(MVFSOpts.forceSpillElements, Int.MaxValue)
    }
    sc = TestUtil.newSparkContext(conf)
  ...
  def testSortBreakingSortingContracts(conf: SparkConf): Unit = {
    ...
    strSorter = new DMOSorter[String, String, String](
  ...
  def testSortWithoutBreakingSortingContracts(conf: SparkConf): Unit = {
    ...
    val testSorter = new DMOSorter[Int, Int, ArrayBuffer[Int]](
  ...
  def testSpillWithHashCollisions(): Unit = {
    ...
    val testSorter = new DMOSorter[String, String, ArrayBuffer[String]](
      context, Some(agg), None, None)
  ...
  def testWithManyHashCollisions(): Unit = {
    ...
    val testSorter = new DMOSorter[FixedHash, Int, Int](
      context, Some(agg), None, None)
