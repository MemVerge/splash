...
import com.memverge.mvfs.dmo.{DMOInputStream, DMOOutputStream, DMOTmpFile}
...
private[spark] class DMOUnsafeShuffleWriter[K, V](
    resolver: DMOShuffleBlockResolver,
    handle: DMOSerializedShuffleHandle[K, V],
    mapId: Int,
    context: TaskContext,
  ...
  private val fileBufferSize = conf.get(
    MVFSOpts.mvfsShuffleFileBufferKB).toInt * 1024
  ...
  private var sorter = new DMOUnsafeSorter(
    context,
  ...
  private var serBuffer = MyByteArrayOutputStream(
    DMOUnsafeShuffleWriter.defaultInitialSerBufferSize)
  ...
  def mergeSpills(spills: Array[ShuffleSpillInfo], dataTmp: DMOTmpFile): Array[Long] = {
    val compressionEnabled = conf.get(MVFSOpts.shuffleCompress)
    val compressionCodec = CompressionCodec.createCodec(conf)
    val fastMergeEnabled = conf.get(MVFSOpts.fastMergeEnabled)
  ...
  private def mergeSpillsWithStream(
    ...
    val mergedOs = new CountingOutputStream(
      new BufferedOutputStream(
        new DMOOutputStream(tmpData, true, true),
        fileBufferSize))

    var threwException = true
    try {
      val dmoIss = spills.indices.map { i =>
        new BufferedInputStream(
          new DMOInputStream(spills(i).file), fileBufferSize)
      }
...
private[spark] object DMOUnsafeShuffleWriter {
