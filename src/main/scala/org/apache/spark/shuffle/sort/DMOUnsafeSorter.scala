...
import org.apache.spark.shuffle.memverge.{DMOObjectWriter, DMOSerializer, MVFSOpts, ShuffleSpillInfo}
 ...
 * DMOShuffleWriter: each output partition's records are
 * written as a single serialized, compressed stream that can be read with a new decompression and
 * deserialization stream.
 * <p>
 * Unlike DMOSorter, this sorter does not merge its
 * spill files. Instead, this merging is performed in DMOUnsafeShuffleWriter, which uses a
 * specialized merge procedure that avoids extra serialization/deserialization.
 */
private[spark] class DMOUnsafeSorter(
    context: TaskContext,
    numPartitions: Int,
  ...
  private val initialSize: Int = conf.get(MVFSOpts.shuffleInitialBufferSize)
  private val numElementsForSpillThreshold: Int = conf.get(MVFSOpts.forceSpillElements)
  private val useRadixSort = conf.get(MVFSOpts.useRadixSort)
  ...
  /**
   * Sorts the in-memory records and writes the sorted records to a DMO file.
  ...
  private def writeSortedFile(isLastFile: Boolean): Unit = {
    ...
    val tmpDmoFile = DMOTmpFile.make(mvfsSocket, dataChunkSize)
    val spilledFile = ShuffleSpillInfo(
      new Array[Long](numPartitions), tmpDmoFile, blockId)
  ...
  /**
   * Sort and spill the current records to DMO in response to memory pressure.
   */
  override def spill(size: Long, trigger: MemoryConsumer): Long = {
    if (trigger != this || inMemSorter == null || inMemSorter.numRecords() == 0) {
      ...
      logInfo(s"Thread ${Thread.currentThread().getId} spilling sort " +
          s"data of ${Utils.bytesToString(getMemoryUsage)} to dmo (" +
  ...
  def cleanupResources(): Unit = {
    ...
    spills.foreach(_.file.forceDelete())
  }
