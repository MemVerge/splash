 ...
 */
class DMOHashComparator[K] extends Comparator[K] {
  def compare(key1: K, key2: K): Int = {
    val hash1 = DMOUtils.hash(key1)
    val hash2 = DMOUtils.hash(key2)
    if (hash1 < hash2) -1 else if (hash1 == hash2) 0 else 1
  }
}


class DMOSpillableIterator[T](var upstream: Iterator[T],
  ...
  def spill(): Option[SpilledFile] = spillLock.synchronized {
    spilledFileOpt match {
      ...
      case None =>
        ...
        val dmoFile = spilledFile.file
        nextUpstream = getNextUpstream(spilledFile)
        logInfo(s"spilling in-memory " +
            s"data structure to dmo ${dmoFile.getDmoId} " +
            s"size ${dmoFile.getSize}.")
