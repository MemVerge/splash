...
/**
 * :: DeveloperApi ::
 * An append-only map that spills sorted content to DMO when there is insufficient space for it
 * to grow.
 *
 * This map takes two passes over the data:
 *
 * (1) Values are merged into combiners, which are sorted and spilled to DMO as necessary
 ...
 * However, if the spill threshold is too low, we spill frequently and incur unnecessary DMO
 * writes. This may lead to a performance regression compared to the normal case of using the
 * non-spilling AppendOnlyMap.
 */
private[spark] class DMOAppendOnlyMap[K, V, C](
    createCombiner: V => C,
    mergeValue: (C, V) => C,
  ...
  private val spilledMaps = new ArrayBuffer[DMOMapIterator]
  ...
  private val keyComparator = new DMOHashComparator[K]
  private var isHugeValueBuffer = false

  @volatile private var readingIterator: DMOSpillableIterator[(K, C)] = _
  ...
  override protected[this] def spill(collection: SizeTracker): Unit = {
    ...
    val dmoFile = spillInMemoryIterator(inMemoryIterator).file
    val dmoMapIterator = new DMOMapIterator(dmoFile)
    spilledMaps += dmoMapIterator
    isHugeValueBuffer = false
    DMOAppendOnlyMap.spilledCount += 1
  }
  ...
  private class DMOIterator extends Iterator[(K, C)] {
  ...
  private class DMOMapIterator(tmpFile: DMOTmpFile) extends Iterator[(K, C)] {
  ...
  private def getNextUpstream(spilledFile: SpilledFile): Iterator[(K, C)] = {
    new DMOMapIterator(spilledFile.file)
  }
  ...
  private def spillInMemoryIterator(inMemoryIterator: Iterator[(K, C)]): SpilledFile = {
    ...
    val blockId = TempLocalBlockId(dmoFile.uuid())
    ...
    try {
      ...
      writer.close()
      _dmoBytesSpilled += dmoFile.getSize
      success = true
    } finally {
    ...
    SpilledFile(dmoFile, blockId, success)
  }

  def destructiveIterator(inMemoryIterator: Iterator[(K, C)]): Iterator[(K, C)] = {
    readingIterator = new DMOSpillableIterator[(K, C)](
      inMemoryIterator,
  ...
  private def hashKey(kc: (K, C)): Int = DMOUtils.hash(kc._1)
...
private[spark] object DMOAppendOnlyMap {
